...
void generate_intermediate_file(const char *filename) {
    FILE *intermediate_file = fopen(filename, "w");
    ...
    for (int i = 0; i < token_count; i++) {
        ...
        if (tokens[i].type == KEYWORD) {
            fprintf(intermediate_file, "kw %s ", tokens[i].token);
        } else if (tokens[i].type == IDENTIFIER) {
            fprintf(intermediate_file, "id %s ", tokens[i].token);
        } else if (tokens[i].type == SEPARATOR || tokens[i].type == UNKNOWN) {
            fprintf(intermediate_file, "%s ", tokens[i].token);
...
int main() {
    ...
    generate_intermediate_file(intermediate_filename);
